...
export async function getSetting(setting: string): Promise<null | string | string[]> {
  if ("locales" === setting) {
...
export async function setSetting(setting: string, value: string): Promise<string> {
...
export async function isLocaleOk(language: string, country: string): Promise<boolean> {
  ...
  return (await getSetting("locales") ?? ["en-US"] as string[]).includes(locale);
}
...
export async function getSettings(): Promise<Settings> {
  ...
  const settings: Settings = {
    ...
    ppc_version: "1.0.0",
    pd_url: pd_url ?? "",
    pd_version: "1.0.0" === pd_version ? "1.0.0" : "2.0.0",
